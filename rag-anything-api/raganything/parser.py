...
class MineruParser(Parser):
    ...
    def _run_ffmpeg_command(self, args: List[str]) -> subprocess.CompletedProcess:
        """Run ffmpeg command with cross-platform support."""
        cmd = ["ffmpeg", "-y"] + args
        subprocess_kwargs = {
            "capture_output": True,
            "text": True,
        ...
        if _IS_WINDOWS:
            subprocess_kwargs["creationflags"] = subprocess.CREATE_NO_WINDOW
        return subprocess.run(cmd, **subprocess_kwargs, check=True)

    def _get_video_duration(self, file_path: Union[str, Path]) -> float:
    ...
    def _transcribe_with_whisper(
        ...
        try:
            ...
            whisper_kwargs = {"language": lang} if lang else {}
            if download_root:
                model = whisper.load_model(
                    model_size, download_root=str(download_root)
                )
            else:
                model = whisper.load_model(model_size)
            result = model.transcribe(str(audio_path), **whisper_kwargs)
            return result.get("text", "")
        except ImportError:
            self.logger.info("Whisper Python API not available, trying CLI...")
        ...
        subprocess.run(cmd, **subprocess_kwargs, check=True)
    ...
    def parse_video(
        ...
        **kwargs,
        ...
        """
        Parse video file by extracting keyframes and transcribing audio.
        ...
        Args:
            ...
            output_dir: Output directory path
            lang: Language for Whisper transcription
            **kwargs: Additional parameters including:
                - video_frame_interval: seconds between keyframes (default 10.0)
                - video_max_frames: max keyframes to extract (default 20)
        ...
        if output_dir:
            ...
            base_output_dir = file_path.parent / f"{file_path.stem}_video_output"
        base_output_dir.mkdir(parents=True, exist_ok=True)

        # Extract audio track
        audio_path = base_output_dir / "extracted_audio.wav"
        self.logger.info(f"Extracting audio from {file_path.name}...")
        ...
        whisper_download_root = kwargs.get("whisper_download_root")
        self.logger.info("Transcribing video audio with Whisper...")
        ...
        interval = float(kwargs.get("video_frame_interval", 10.0))
        max_frames = int(kwargs.get("video_max_frames", 20))

        frames_dir = base_output_dir / "frames"
        ...
        if duration <= 0:
            ...
            actual_interval = duration / max(1, num_frames - 1) if num_frames > 1 else 0

        frame_paths = []
        for i in range(num_frames):
            timestamp = i * actual_interval
            # Ensure timestamp is strictly within video duration to avoid ffmpeg errors
            if duration > 0:
                timestamp = min(timestamp, max(0.0, duration - 0.5))
            frame_path = frames_dir / f"frame_{i:03d}.jpg"
            self.logger.info(f"Extracting frame at {timestamp:.1f}s...")
            try:
                self._run_ffmpeg_command(
                    [
                        "-ss",
                ...
                self.logger.warning(
                    f"Failed to extract frame at {timestamp:.1f}s: {e}. Skipping."
                )
        ...
        # Build content_list in MinerU format
            ...
            content_list.append(
                {
                    ...
                    "image_caption": [f"Video frame @ {timestamp:.1f}s"],
                    "page_idx": 0,
                }
            )

        self.logger.info(
            f"Video parsing complete: {len(content_list)} blocks "
            f"({len(frame_paths)} frames, transcript length: {len(transcript)} chars)"
        )
        return content_list
    ...
    def parse_audio(
        ...
        **kwargs,
        ...
        """
        Parse audio file by transcribing with Whisper.
        ...
        if output_dir:
            ...
            base_output_dir = file_path.parent / f"{file_path.stem}_audio_output"
        base_output_dir.mkdir(parents=True, exist_ok=True)

        # Convert to WAV for Whisper if not already WAV
        if file_path.suffix.lower() == ".wav":
            audio_path = file_path
        else:
            audio_path = base_output_dir / "converted_audio.wav"
            self.logger.info(f"Converting {file_path.name} to WAV...")
            self._run_ffmpeg_command(
                [
                    "-i",
                    str(file_path),
        ...
        self.logger.info(
            f"Audio parsing complete: {len(content_list)} blocks "
    ...
    def parse_document(
        ...
        **kwargs,
        ...
        if ext == ".pdf":
            ...
            return self.parse_video(file_path, output_dir, lang, **kwargs)
