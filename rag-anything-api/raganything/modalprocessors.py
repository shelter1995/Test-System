...
class VideoModalProcessor(BaseModalProcessor):
    ...
    async def generate_description_only(
        ...
        try:
            ...
            # Build video analysis prompt with context
            if context:
                video_prompt = PROMPTS.get(
                    ...
                    entity_name=entity_name
                    if entity_name
                    else "descriptive name for this video",
            ...
            response = await self.modal_caption_func(
                ...
                system_prompt=PROMPTS.get(
                    ...
                    "You are an expert video analyst. Provide detailed, accurate analysis of video content.",
                ),
            )
    ...
    def _parse_video_response(
        ...
        try:
            ...
            fallback_entity = {
                "entity_name": entity_name
                if entity_name
                else f"video_{compute_mdhash_id(cleaned)}",
                "entity_type": "video",
                "summary": cleaned[:100] + "..." if len(cleaned) > 100 else cleaned,
            }
            return cleaned, fallback_entity
...
class AudioModalProcessor(BaseModalProcessor):
    ...
    async def generate_description_only(
        ...
        try:
            ...
            # Build audio analysis prompt with context
            if context:
                audio_prompt = PROMPTS.get(
                    ...
                    entity_name=entity_name
                    if entity_name
                    else "descriptive name for this audio",
            ...
            response = await self.modal_caption_func(
                ...
                system_prompt=PROMPTS.get(
                    ...
                    "You are an expert audio analyst. Provide detailed analysis of audio content and transcripts.",
                ),
            )

            # Parse response
            enhanced_caption, entity_info = self._parse_audio_response(
                response, entity_name
            )
    ...
    async def process_multimodal_content(
        ...
        try:
            ...
            # Parse audio content for building complete chunk
            if isinstance(modal_content, str):
                try:
                    content_data = json.loads(modal_content)
                except json.JSONDecodeError:
    ...
    def _parse_audio_response(
        ...
        try:
            ...
            fallback_entity = {
                "entity_name": entity_name
                if entity_name
                else f"audio_{compute_mdhash_id(cleaned)}",
                "entity_type": "audio",
                "summary": cleaned[:100] + "..." if len(cleaned) > 100 else cleaned,
            }
            return cleaned, fallback_entity
