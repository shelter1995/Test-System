...
# 视频分析提示模板
PROMPTS_ZH["video_prompt"] = """请分析此视频内容，并以以下JSON结构提供回答：

{{
    "detailed_description": "对视频的全面分析，包括：
    - 整体内容摘要和主要主题
    - 关键视觉元素和场景
    - 重要的对话或旁白
    - 时间线和叙事结构
    ...
    "entity_info": {{
        "entity_name": "{entity_name}",
        "entity_type": "video",
        "summary": "视频目的和关键内容的简明摘要（不超过100字）"
    }}
}}
...
请专注于提取有意义的洞察和对视频内容的整体理解。"""

# 带上下文的视频分析提示模板
PROMPTS_ZH["video_prompt_with_context"] = """请结合上下文分析此视频内容，并以以下JSON结构提供回答：
...
{{
    ...
    "entity_info": {{
        "entity_name": "{entity_name}",
        "entity_type": "video",
        "summary": "视频目的、关键内容及与周围上下文关系的简明摘要（不超过100字）"
    }}
}}
...
请专注于在上下文背景下提取有意义的洞察和对视频内容的整体理解。"""

# 音频分析提示模板
PROMPTS_ZH["audio_prompt"] = """请分析此音频内容，并以以下JSON结构提供回答：

{{
    "detailed_description": "对音频的全面分析，包括：
    - 整体内容摘要和主要主题
    - 关键口语内容和对话
    - 说话人识别和角色（如可辨别）
    - 语气、情绪和情感内容
    ...
    "entity_info": {{
        "entity_name": "{entity_name}",
        "entity_type": "audio",
        "summary": "音频目的和关键内容的简明摘要（不超过100字）"
    }}
}}
...
# 带上下文的音频分析提示模板
PROMPTS_ZH["audio_prompt_with_context"] = """请结合上下文分析此音频内容，并以以下JSON结构提供回答：
...
{{
    ...
    "entity_info": {{
        "entity_name": "{entity_name}",
        "entity_type": "audio",
        "summary": "音频目的、关键内容及与周围上下文关系的简明摘要（不超过100字）"
    }}
}}
...
请专注于在上下文背景下从音频内容中提取有意义的洞察。"""

# 视频和音频的 chunk 模板
PROMPTS_ZH["video_chunk"] = """视频分析：
视频路径：{video_path}
时长：{duration}
关键帧标注：{captions}
转录文本：{transcript}
