...
class RAGAnythingConfig:
    ...
    video_frame_interval: float = field(
        default=get_env_value("VIDEO_FRAME_INTERVAL", 10.0, float)
    )
    """Interval in seconds between extracted video keyframes."""

    video_max_frames: int = field(
        default=get_env_value("VIDEO_MAX_FRAMES", 20, int)
    )
    """Maximum number of keyframes to extract from a single video."""

    whisper_model: str = field(
        default=get_env_value("WHISPER_MODEL", "base", str)
    )
    ...
    supported_file_extensions: List[str] = field(
        default_factory=lambda: [
            ...
            for x in get_env_value(
                ...
                ".pdf,.jpg,.jpeg,.png,.bmp,.tiff,.tif,.gif,.webp,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.txt,.md,.mp4,.avi,.mkv,.mov,.webm,.mp3,.wav,.flac,.aac,.ogg,.m4a",
                str,
            ).split(",")
        ]
    )
