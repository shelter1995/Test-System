...
def test_import(file_path: str, database: str = "商务彩铃"):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="测试文件导入到 RAG-Anything")
    parser.add_argument("file_path", help="测试文件路径")
    parser.add_argument("--database", "-d", default="商务彩铃", help="数据库标识")
    args = parser.parse_args()

    result = test_import(args.file_path, args.database)
