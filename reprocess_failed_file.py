...
def reprocess_file(file_path: str, database: str = "商务彩铃"):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="重新导入指定文件到 RAG-Anything 知识库")
    parser.add_argument("file_path", help="要重新导入的文件路径")
    parser.add_argument("--database", "-d", default="商务彩铃", help="数据库标识")
    args = parser.parse_args()

    result = reprocess_file(args.file_path, args.database)
